...
import { ReportGenerationJobData, ExportFormat } from '../services/report-queue.service';
...
export class ReportQueueProcessor {
  ...
  async handlePdfGeneration(job: Job<ReportGenerationJobData>) {
    const { reportId, reportType, startDate, endDate, siteId, provinceId, generatedBy, reportName, reportOptions, contentMetadata } = job.data;
    ...
    try {
      ...
      try {
        ...
        const fileName = `report_${reportId}_${Date.now()}.pdf`;
        fileUrl = await this.fileStorageService.uploadToLocalStorage(fileName, pdfBuffer);
      }
  ...
  async handleExcelGeneration(job: Job<ReportGenerationJobData>) {
    ...
    try {
      ...
      let fileUrl: string;
      try {
        fileUrl = await this.fileStorageService.uploadExcel(exportResult.filename, exportResult.buffer);
      } catch (s3Error) {
        this.logger.warn('S3 upload failed, falling back to local storage:', s3Error);
        fileUrl = await this.fileStorageService.uploadToLocalStorage(exportResult.filename, exportResult.buffer);
      }
      ...
      this.logger.error(`Excel generation failed for report ${reportId}:`, error);
  ...
  async handleBothGeneration(job: Job<ReportGenerationJobData>) {
    ...
    this.logger.log(`Starting both PDF and Excel generation for report ${reportId}`);
    ...
    try {
      ...
      const [pdfResult, excelResult] = await Promise.all([
        this.generatePdf(reportId, reportType, startDate, endDate, siteId, provinceId, generatedBy, reportName, reportOptions, contentMetadata),
        this.generateExcel(reportId, reportType, startDate, endDate, siteId, provinceId, generatedBy, reportName, reportOptions, contentMetadata)
      ]);
      ...
      this.logger.log(`Both PDF and Excel generation completed for report ${reportId}`);
      ...
      this.logger.error(`Both generation failed for report ${reportId}:`, error);
  ...
  private async generatePdf(reportId: string, reportType: ReportType, startDate: string, endDate: string, siteId?: string, provinceId?: string, generatedBy?: string, reportName?: string, reportOptions?: any, contentMetadata?: any) {
  ...
  private async generateExcel(reportId: string, reportType: ReportType, startDate: string, endDate: string, siteId?: string, provinceId?: string, generatedBy?: string, reportName?: string, reportOptions?: any, contentMetadata?: any) {
    // Generate Excel content using unified export service
    const exportResult = await this.exportService.exportReport({
      reportType,
      exportFormat: ExportFormat.EXCEL,
